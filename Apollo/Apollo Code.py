...
myClock=time.Clock()
titleScreen=image.load('Pictures/Title Screen.png')
titleBack=transform.scale(titleScreen,(800,600))
...
classScreen=image.load('Pictures/Class Selection Background.jpg')
classBack=transform.scale(classScreen,(800,600))
...
al=image.load('Pictures/alien 1.png')
pos=''
...
def badMove(bguy,x,y):
    ...
    moveX=(x-bguy[0])*0.1/dist
    moveY=(y-bguy[1])*0.1/dist
...
def gameLoad():
    ...
    newRect=Rect(150,425,198,45)
    loadRect=Rect(450,425,198,45)
        ...
        for evnt in event.get():
            ...
            if evnt.type==KEYDOWN:
                if evnt.key==K_ESCAPE:
                    gameLoadRunning=False
        screen.blit(titleBack,(0,0))
        screen.blit(newButton,(150,425))
        screen.blit(loadButton,(450,425))
...
def room_1():
    ...
    global gunHeat
    global pos
    player=transform.scale(guy[pos],(35,35))
    myClock.tick(1000)
    ...
    while room1Running:
        for evnt in event.get():
            ...
            if evnt.type==KEYDOWN:
                if evnt.key==K_ESCAPE:
                    room1Running=False
        screen.blit(firstBack,(0,0))
        screen.blit(player,(guyx,guyy))
        for shot in shots[:]:
            draw.circle(screen,(30,30,255),(int(shot[X]),int(shot[Y])),3)
            used = kill(shot[X],shot[Y])
            if used:
                shots.remove(shot)
        for bguy in badGuys:
        ...
        if keys[SPACE] and gunHeat<=0:
            gunHeat=20
            if pos=="R":
                shots.append(addShot(0,power))
            if pos=="L":
                shots.append(addShot(180,power))
            if pos=="":
                shots.append(addShot(270,power))
            if pos=="D":
                shots.append(addShot(90,power))
        gunHeat -= 1
        moveShots(shots)
        moveGuy(guyx,guyy)
        display.flip()
